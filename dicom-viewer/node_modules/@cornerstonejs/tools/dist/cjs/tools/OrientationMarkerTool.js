    ...
    _unsubscribeToViewportNewVolumeSet() {
        const unsubscribe = () => {
            ...
            viewportsInfo.forEach(({ viewportId, renderingEngineId }) => {
                ...
                element.removeEventListener(core_1.Enums.Events.VOLUME_VIEWPORT_NEW_VOLUME, this.initViewports.bind(this));
                const resizeObserver = this._resizeObservers.get(viewportId);
                resizeObserver.unobserve(element);
            });
        };
        core_1.eventTarget.removeEventListener(enums_1.Events.TOOLGROUP_VIEWPORT_ADDED, (evt) => {
    ...
    _subscribeToViewportEvents() {
        const subscribeToElementResize = () => {
            ...
            viewportsInfo.forEach(({ viewportId, renderingEngineId }) => {
                ...
                const { element } = viewport;
                this.initViewports();
                element.addEventListener(core_1.Enums.Events.VOLUME_VIEWPORT_NEW_VOLUME, this.initViewports.bind(this));
                const resizeObserver = new ResizeObserver(() => {
                    setTimeout(() => {
                        const element = (0, core_1.getEnabledElementByIds)(viewportId, renderingEngineId);
                        if (!element) {
                            return;
                        }
        ...
        core_1.eventTarget.addEventListener(enums_1.Events.TOOLGROUP_VIEWPORT_ADDED, (evt) => {
            if (evt.detail.toolGroupId !== this.toolGroupId) {
                return;
            }
            subscribeToElementResize();
