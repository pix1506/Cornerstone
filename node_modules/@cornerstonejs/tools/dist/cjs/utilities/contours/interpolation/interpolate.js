...
const createPolylineToolData_1 = __importDefault(require("./createPolylineToolData"));
...
const updateChildInterpolationUID_1 = __importDefault(require("./updateChildInterpolationUID"));
const contourSegmentationCompleted_1 = require("../../../eventListeners/annotations/contourSegmentation/contourSegmentationCompleted");
...
function startInterpolation(viewportData) {
    ...
    (0, updateChildInterpolationUID_1.default)(toolData);
    const { interpolationData, interpolationList } = (0, findAnnotationForInterpolation_1.default)(toolData, viewportData) || {};
    ...
    if (interpolationList.length) {
        (0, core_1.triggerEvent)(viewportData.viewport.element, Events_1.default.ANNOTATION_INTERPOLATION_PROCESS_COMPLETED, eventDetails);
    }
}
...
function _addInterpolatedContour(interpolated3DPoints, handlePoints, sliceIndex, referencedToolData, eventData) {
    ...
    if (parentAnnotationUID) {
        ...
        (0, contourSegmentationCompleted_1.createPolylineHole)(viewport, parentAnnotation, interpolatedAnnotation);
    }
}
...
function _editInterpolatedContour(interpolated3DPoints, handlePoints, sliceIndex, referencedToolData, eventData) {
    ...
    const points = interpolated3DPoints.points;
    const interpolatedAnnotation = (0, createPolylineToolData_1.default)(points, handlePoints, oldAnnotationData);
    ...
    for (let startingNode = 0; startingNode < c1iLength; startingNode++) {
        ...
        for (let iteration = 0; iteration < c1iLength; iteration++) {
            totalSquaredXYLengths +=
                Math.pow((c1i.x[node] - c2i.x[iteration]), 2) +
                    Math.pow((c1i.y[node] - c2i.y[iteration]), 2) +
                    Math.pow((c1i.z[node] - c2i.z[iteration]), 2);
            node++;
            if (node === c1iLength) {
                node = 0;
            }
...
function _getCumulativePerimeter(contour) {
    ...
    for (let i = 1; i < contour.x.length; i++) {
        const lengthOfSegment = Math.sqrt(Math.pow((contour.x[i] - contour.x[i - 1]), 2) +
            Math.pow((contour.y[i] - contour.y[i - 1]), 2) +
            Math.pow((contour.z[i] - contour.z[i - 1]), 2));
